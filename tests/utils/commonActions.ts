...
export class CommonActions {
    ...
    /**
    * Navigates to the specified URL to launch the application
    * 
    * @param {string} url - The URL of the application to load.
    */
  ...
  async getLocator(
    ...
    /**
    * Interacts with a web element based on the given role name and text name.
    *
    * @param {string} roleName - The type of locator to use ("ROLE").
    * @param {string} textName - The value of the text to find the element.
    ...
    * @throws {Error} Throws an error if an unsupported attribute or action is used.
    */
    async getByRoleTag(roleName: "button" | "link", textName: string, action: "click" | "fill", data: string = "") {
        const locator = this.page.getByRole(roleName, { name: textName })
        if (action === "fill" && !data) {
    ...
    async interactWithElement(
        attribute: "LABEL" | "PLACEHOLDER" | "TEXT" | "TITLE" | "ALTTEXT" | "ID" | "CLASS" | "XPATH",
        locator: string,
        action: "click" | "fill",
        data: string = ""
    ) {
